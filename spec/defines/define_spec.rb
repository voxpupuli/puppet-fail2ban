  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'when source file' do
        let(:params) do
          {
            config_file_source: 'puppet:///modules/fail2ban/Debian/12/etc/fail2ban/jail.conf'
          }
        end
        ...
        it do
          is_expected.to contain_file('define_fail2ban.conf').with(
            'ensure' => 'present',
            'source' => 'puppet:///modules/fail2ban/Debian/12/etc/fail2ban/jail.conf',
      ...
      context 'when content template' do
        let(:params) do
          {
            config_file_template: 'fail2ban/Debian/12/etc/fail2ban/jail.conf.epp'
          }
        end
